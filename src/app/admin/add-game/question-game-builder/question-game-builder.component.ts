import {
  ...
  EventEmitter, Output, ChangeDetectorRef, OnDestroy, HostListener, ChangeDetectionStrategy
} from '@angular/core';
...
@Component({
  selector: 'app-question-game-builder',
  templateUrl: './question-game-builder.component.html',
  styleUrls: ['./question-game-builder.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,

})
export class QuestionGameBuilderComponent implements OnInit, OnChanges, OnDestroy {
  @Input() questions;
  @Input() addQuestion;
  @Input() isCategory;
  @Input() isGame;
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes.openCustomMenu && changes.openCustomMenu.currentValue) {
    ...
    if (changes.questions && !changes.questions.firstChange) {
      ...
      if (previousQuestions?.length && previousQuestions[0].is_new) {
      }
      if (this.category) {
      ...
      this.questionToggled();
      if (!this.isGame && this.selectedChip !== -2) {
  ...
  cloneQuestion(index) {
    this.addNewQuestionRowForDuplicate(12345, index);
  ...
  addNewQuestionRowForDuplicate(isDuplicate = null, index = 0) {
    ...
    if (isDuplicate) {
      ...
      data.splice(index, 0, question); // added to one above the selected question index
      this.questionBeingEdited = question;
      this.selectedGameIdForAddQuestion = question.game_id;
      this.selectedCategoryIdForAddQuestion = question.category_id;
      this.questions = [...data];
  ...
  addNewQuestionRow(isDuplicate = null, index = 0) {
    ...
    this.allLanguage &&
    this.allLanguage.selected_language.filter((x) => x.id == selectedLanguage);
    question["language"] = selectedLangObject[0];
  ...
  getSelectedQuestionAll(event) {
    ...
    if (event.checked) {
    const idKey = this.isfeedback ? 'question_feedback_id' : 'question_id';
    this.questions.forEach((question, i) => {
      if (!question.is_shop_game && !question.is_archived) {
        this.selectedQuestionsList[i] = question[idKey];
      }
    });
  }
  this.checkBoxAction.emit(this.selectedQuestionsList);
  ...
  selectedQuestion(question) {
    ...
    if (question.is_checked) {
      this.selectedQuestionsList.push(question[idKey]);
      this.checked = this.questions.length === this.selectedQuestionsList.length;
    } else {
      this.selectedQuestionsList = this.selectedQuestionsList.filter(selectedQuestion => question[idKey] !== +selectedQuestion);
      this.checked = false;
    }
  
    this.checkBoxAction.emit(this.selectedQuestionsList);
  }
  handleQuestionChecked(question: any) {
