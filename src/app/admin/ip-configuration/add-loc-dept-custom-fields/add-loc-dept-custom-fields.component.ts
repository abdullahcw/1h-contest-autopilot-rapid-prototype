...
@Component({
  selector: 'app-add-loc-dept-custom-fields',
  templateUrl: './add-loc-dept-custom-fields.component.html',
  styleUrls: ['./add-loc-dept-custom-fields.component.scss']
})
export class AddLocDeptCustomFieldsComponent implements OnInit {
  menuList: any[];
  ...
  ngOnInit() {
    ...
    this.addRemoveFilterOption(this.filters);
  ...
  addRemoveFilterOption(appliedFilters) {
    ...
    console.log('hiiiii',customCount)

    if (customCount && customCount.length > 0) {
      console.log('hiiiii')
      this.filter_options.forEach(option => {
        console.log('hiiiii',option)
  ...
  getDepartments() {
    ...
    this.departmentService.getDepartments(this.companyId, 'department_name', 'asc', 0, 0, filters, false).subscribe((res) => {
      ...
      this.departmentList = this.globalService.prepareSelectionList(deptList, filterInfo, this.filters, forceSelection);
  ...
  cancel(isChanged = false) {
    ...
    this.globalService.addAdminGoogleEvent('IP_Loc_Dept_Save');
